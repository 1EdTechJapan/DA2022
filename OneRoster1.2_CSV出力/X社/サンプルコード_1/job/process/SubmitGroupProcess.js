...
import { deleteClass } from '../../helpers/lgateApi.js';
import LogHelper from "../../helpers/log.js";
import { deleteUsersImport } from "./SubmitClassesProcess.js";
const LOG = new LogHelper(OTHER_CLASS_COLLATION);
 ...
 * @constructor
 */
export const mergedUsersGroup = async (municipalId,role,classType) => {
            ...
            if (row.importClassUsers && row.importOrgsId) {
                ...
                if (row.role === "student" || row.role === "teacher") {
                    ...
                    if (arrUCEnr[key] === "homeroom") {
                        ...
                        if (arrUserEnrSubmited.includes(arrUCEnr[key]+":1")) {
...
/**
 * Processing bull job for submited data from group import process
 * @constructor
 */
class SubmitClasses {
    onProcess = async (job,done,initData = null,stepIndex=0, limit=100, offset=0) => {
        ...
        switch(step[stepIndex]){
            case 'import_table_update': 
                ...
                initData.logData = {logFor: OTHER_CLASS_COLLATION,message:lang.t('submit_group.message.csv_import_data_updated'), executionTime : initData.insertTime}
            ...
            case 'import_merge_data':
                ...
                const query = `
                    ...
                    WHERE
                        import_classes.classType = 'scheduled'
                ...
                if(queryResult.length){
                    ...
                    await Promise.all(
                        importCsvData.map(async (item) => {
                            ...
                            if(item.option == 'add_from_lgate') console.log('adaloh');
                    ...
                    initData.progress = resultImportCsvDataSubmited && Array.isArray(resultImportCsvDataSubmited) ? resultImportCsvDataSubmited.length : 0
                    ...
                    initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                    initData.logData = {logFor: OTHER_CLASS_COLLATION,message:lang.t('submit_group.message.csv_data_merged_process').replace('%ROW_START%',rowstart).replace('%ROW_END%',rowend), executionTime : initData.insertTime}
                    LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            ...
            case 'update_tobedeleted': 
                ...
                const queryTobeDeleted = `
                    ...
                    WHERE
                        ...
                        AND classes.classType = 'scheduled'
                ...
                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: OTHER_CLASS_COLLATION,message:lang.t('submit_group.message.roster_data_status_tobedeleted'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            ...
            case 'count_all_data_import' :
                ...
                initData.importCsvDataAll = resultImportCsvDataAll && Array.isArray(resultImportCsvDataAll) ? resultImportCsvDataAll.length : 0
                await job.progress({total_row:initData.importCsvDataAll,progress_row:initData.progress,is_calculating:true})
                ++stepIndex

                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: OTHER_CLASS_COLLATION,message:lang.t('submit_group.message.count_all_csv_import_data'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            break;
            
            default:
                initData.startInsertTime = moment()
                await mergedUsers(param.municipalId,["teacher", "student","administrator"],"scheduled")

                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: OTHER_CLASS_COLLATION,message:lang.t('submit_group.message.matching_csv_data'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
    ...
    onActive = async (job, jobPromise) => {
        let logData = {logFor: OTHER_CLASS_COLLATION,message:lang.t('submit_group.message.submit_merged_start')}
    ...
    onCompleted = async (job,result) => {
        ...
        const importTaskProgressId = `${job.data.municipalId}_${OTHER_CLASS_COLLATION}`
        ...
        if(job.data && job.data.schoolSourcedId){
            ...
            await db[job.data.municipalId].importTaskProgressDetail.upsert({
                id: `${job.data.municipalId}_${job.data.schoolSourcedId}_${OTHER_CLASS_COLLATION}`,
        ...
        if(job.data.schoolSourcedId && resultOrgSubmited.length == 0){
            const [groupAlreadyToSubmit] = await db[job.data.municipalId].query(
                `SELECT COUNT(sourcedId) as count FROM import_classes WHERE municipalId = '${job.data.municipalId}' and classType='scheduled' and submited='1' and isParentSkiped='0' LIMIT 1`
            )
            const [groupHaveToSubmit] = await db[job.data.municipalId].query(
                `SELECT COUNT(sourcedId) as count FROM import_classes WHERE municipalId = '${job.data.municipalId}' and classType='scheduled' and isParentSkiped='0' LIMIT 1`
            )
        
            const countGroupSubmited = parseInt(groupAlreadyToSubmit[0]?.count || 0)
            const countGroupImport = parseInt(groupHaveToSubmit[0]?.count || 0)

            percent = countGroupImport < 1 ? 100 : (countGroupSubmited/countGroupImport)*100;
        }
        ...
        let logData = {logFor: OTHER_CLASS_COLLATION,message:lang.t('submit_group.message.submit_merged_finish')}
        LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData});
        console.log('completeds',job.id, result)
    }

    onFailed = async (job, err) => {
        let logData = {logFor: OTHER_CLASS_COLLATION,message:lang.t('submit_group.message.submit_merged_failed').replace('%MESSAGE%',err.message)}
        LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData});
