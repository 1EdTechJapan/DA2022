...
import e from 'cors';
import { excludeArrayValue, mappingQueryArrayReturn, convertNullToString, filterUsersSubmitSetting, setPasswordRule } from '../../helpers/utility.js';
import LogHelper from "../../helpers/log.js";
const LOG = new LogHelper(OTHER_ATENDANCE_COLLATION);
...
/**
 * Processing bull job for submited data from submit user group enrollment import process
 * @constructor
 */
class SubmitUsersEnrollmentsGroup {
    onProcess = async (job,done,initData = null,stepIndex=0, limit=100, offset=0) => {
        ...
        switch(step[stepIndex]){
            ...
            initData.startInsertTime = moment()
                ...
                initData.logData = {logFor: OTHER_ATENDANCE_COLLATION,message:lang.t('submit_users_group.csv_import_data_updated'), executionTime : initData.insertTime}
            ...
            case 'import_merge_data':
                ...
                const query = `
                    ...
                    AND import_classes.classType = 'scheduled'
                ...
                if(queryResult.length){
                    ...
                    initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                    initData.logData = {logFor: OTHER_ATENDANCE_COLLATION,message:lang.t('submit_users_group.csv_data_merged_process').replace('%ROW_START%',rowstart).replace('%ROW_END%',rowend), executionTime : initData.insertTime}
                    LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
                } else {
                    ++stepIndex
                    offset = 0
            ...
            initData.startInsertTime = moment()
                ...
                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: OTHER_ATENDANCE_COLLATION,message:lang.t('submit_users_group.roster_data_status_tobedeleted'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            break;
            ...
            case 'count_all_data_import' :
                ...
                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: OTHER_ATENDANCE_COLLATION,message:lang.t('submit_users_group.count_all_csv_import_data'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            break;
    ...
    onActive = async (job, jobPromise) => {
        let logData = {logFor: OTHER_ATENDANCE_COLLATION,message:lang.t('submit_users_group.submit_merged_start')}
    ...
    onCompleted = async (job,result) => {
        ...
        const importTaskProgressId = `${job.data.municipalId}_${OTHER_ATENDANCE_COLLATION}`
        ...
        if(job.data && job.data.schoolSourcedId){
            ...
            await db[job.data.municipalId].importTaskProgressDetail.upsert({
                id: job.data.classSourceId ? `${job.data.municipalId}_${job.data.classSourceId}_${OTHER_ATENDANCE_COLLATION}` : `${job.data.municipalId}_${job.data.schoolSourcedId}_${OTHER_ATENDANCE_COLLATION}`,
        ...
        if(job.data.classSourceId)
            queryall = queryall+` AND rel_classes.importSourcedId = '${job.data.classSourceId}'`
        ...
        if(job.data.schoolSourcedId){
            const [classHasbeenSubmited] = await db[job.data.municipalId].query(`SELECT orgSourcedId, classSourceId, progress FROM import_task_progress_details WHERE municipalId = '${job.data.municipalId}' AND type = '${OTHER_ATENDANCE_COLLATION}'`)
                
            const [classHaveToSubmit] = await db[job.data.municipalId].query(
                `SELECT COUNT(sourcedId) as count, schoolSourcedId FROM import_classes WHERE municipalId = '${job.data.municipalId}' and import_classes.option != 'skip_csv_to_lgate' and import_classes.classType='scheduled' and import_classes.isParentSkiped='0' group by schoolSourcedId`
            )
        ...
        let logData = {logFor: OTHER_ATENDANCE_COLLATION,message:lang.t('submit_users_group.submit_merged_finish')}
        LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData});
        console.log('completeds',job.id, result)
    }

    onFailed = async (job, err) => {
        let logData = {logFor: OTHER_ATENDANCE_COLLATION,message:lang.t('submit_users_group.submit_merged_failed').replace('%MESSAGE%',err.message)}
        LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData});
...
/**
 * Create export variable to running function SubmitUsersEnrollmentsGroup
 * @constructor
 */
const SubmitUsersEnrollmentsGroupProcess = new SubmitUsersEnrollmentsGroup()

export default SubmitUsersEnrollmentsGroupProcess