...
const LOG = new LogHelper(STUDENT_COLLATION);
...
/**
 * Processing bull job for submited data from submit student import process
 * @constructor
 */
class SubmitStudent {
    onProcess = async (job,done,initData = null,stepIndex=0, limit=100, offset=0) => {
        ...
        switch(step[stepIndex]){
            case 'import_table_update': 
                ...
                initData.logData = {logFor: STUDENT_COLLATION,message:lang.t('submit_student.message.csv_import_data_updated'), executionTime : initData.insertTime}
                ...
                if(queryResult.length){
                    ...
                    if (bulkDeleteCsv.length) {
                        ...
                        await db[param.municipalId].relEnrollments.destroy({
                            where: {
                                importUserSourcedId: { [Op.in]: bulkDeleteCsv },isTemp: 1
                            },
                        });
                    }
                    ...
                    initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                    initData.logData = {logFor: STUDENT_COLLATION,message:lang.t('submit_student.message.csv_data_merged_process').replace('%ROW_START%',rowstart).replace('%ROW_END%',rowend), executionTime : initData.insertTime}
                    LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            ...
            case 'update_tobedeleted': 
                ...
                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: STUDENT_COLLATION,message:lang.t('submit_student.message.roster_data_status_tobedeleted'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            ...
            case 'count_all_data_import' :
                ...
                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: STUDENT_COLLATION,message:lang.t('submit_student.message.count_all_csv_import_data'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            break;
    ...
    onActive = async (job, jobPromise) => {
        let logData = {logFor: STUDENT_COLLATION,message:lang.t('submit_student.message.submit_merged_start')}
    ...
    onCompleted = async (job,result) => {
        ...
        const importTaskProgressId = `${job.data.municipalId}_${STUDENT_COLLATION}`

        // HANDLE LEVEL TEACHER AND HEAD PROGRESSBAR -- start
        if(job.data && job.data.schoolSourcedId){
                ...
                await db[job.data.municipalId].importTaskProgressDetail.upsert({
                    id: `${job.data.municipalId}_${job.data.schoolSourcedId}_${STUDENT_COLLATION}`,
        ...
        // HANDLE LEVEL TEACHER AND HEAD PROGRESSBAR -- end
        ...
        // IF BULK ALL ORG AND SUPERADMIN 
        ...
        if(job.data.schoolSourcedId){
            const [orgAlreadySubmit] = await db[job.data.municipalId].query(
                ...
                import_task_progress_details.type = '${STUDENT_COLLATION}' AND import_task_progress_details.municipalId = '${job.data.municipalId}' AND
        ...
        let logData = {logFor: STUDENT_COLLATION,message:lang.t('submit_student.message.submit_merged_finish')}
        LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData});
    ...
    onFailed = async (job, err) => {
        ...
        let logData = {logFor: STUDENT_COLLATION,message:lang.t('submit_student.message.submit_merged_failed').replace('%MESSAGE%',err.message)}
        LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData});
...
/**
 * Create export variable to running function SubmitStudent
 * @constructor
 */
const SubmitSubmitStudentProcess = new SubmitStudent()

export default SubmitSubmitStudentProcess