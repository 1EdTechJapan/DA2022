...
const LOG = new LogHelper(ADMIN_TEACHER_COLLATION);
...
/**
 * Processing bull job for submited data from submit admin teachers import process
 * @constructor
 */
class SubmitAdminTeacher {
    onProcess = async (job,done,initData = null,stepIndex=0, limit=100, offset=0) => {
        ...
        switch(step[stepIndex]){
            case 'import_table_update': 
                ...
                initData.logData = {logFor: ADMIN_TEACHER_COLLATION,message:lang.t('submit_admin_teacher.message.csv_import_data_updated'), executionTime : initData.insertTime}
            ...
            case 'import_merge_data':
                ...
                const query = `
                    ...
                    WHERE 
                    import_users.role IN ("administrator","teacher")
                ...
                if(queryResult.length){
                    ...
                    initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                    initData.logData = {logFor: ADMIN_TEACHER_COLLATION,message:lang.t('submit_admin_teacher.message.csv_data_merged_process').replace('%ROW_START%',rowstart).replace('%ROW_END%',rowend), executionTime : initData.insertTime}
                    LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            ...
            case 'update_tobedeleted': 
                ...
                const queryTobeDeleted = `
                    ...
                    WHERE 
                        users.role IN ("administrator","teacher")
                ...
                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: ADMIN_TEACHER_COLLATION,message:lang.t('submit_admin_teacher.message.roster_data_status_tobedeleted'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            ...
            case 'count_all_data_import' :
                ...
                initData.insertTime = moment().diff(initData.startInsertTime,"seconds",true)
                initData.logData = {logFor: ADMIN_TEACHER_COLLATION,message:lang.t('submit_admin_teacher.message.count_all_csv_import_data'), executionTime : initData.insertTime}
                LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData:initData.logData});
            break;
    ...
    onActive = async (job, jobPromise) => {
        let logData = {logFor: ADMIN_TEACHER_COLLATION,message:lang.t('submit_admin_teacher.message.submit_merged_start')}
    ...
    onCompleted = async (job,result) => {
        ...
        const importTaskProgressId = `${job.data.municipalId}_${ADMIN_TEACHER_COLLATION}`
        ...
        if(job.data && job.data.schoolSourcedId){
            ...
            await db[job.data.municipalId].importTaskProgressDetail.upsert({
                id: `${job.data.municipalId}_${job.data.schoolSourcedId}_${ADMIN_TEACHER_COLLATION}`,
        ...
        const queryall = `
            SELECT 
                import_users.sourcedId, import_users.submited , import_users.municipalId as usermunicipalId , import_users.orgSourcedIds as userOrgSourceId
            FROM 
            import_users
            INNER JOIN import_enrollments ON import_enrollments.userSourcedId = import_users.sourcedId
            INNER JOIN rel_orgs ON rel_orgs.importSourcedId = import_users.orgSourcedIds
        ...
        // HANDLE BOBOT OTHER SCHOOL HAS NO DATA
        if(job.data.schoolSourcedId){
            ...
            const [orgAlreadySubmit] = await db[job.data.municipalId].query(
                 `SELECT SUM(progress) as avarage, COUNT(id) as count
                    ...
                    INNER JOIN import_orgs ON import_orgs.sourcedId COLLATE utf8mb4_general_ci = import_task_progress_details.orgSourcedId WHERE
                    import_task_progress_details.type = '${ADMIN_TEACHER_COLLATION}' AND import_task_progress_details.municipalId = '${job.data.municipalId}' AND
        ...
        let logData = {logFor: ADMIN_TEACHER_COLLATION,message:lang.t('submit_admin_teacher.message.submit_merged_finish')}
        LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData});
    }
    ...
    onFailed = async (job, err) => {
        ...
        let logData = {logFor: ADMIN_TEACHER_COLLATION,message:lang.t('submit_admin_teacher.message.submit_merged_failed').replace('%MESSAGE%',err.message)}
        LOG.create({type: LOG_TYPE_SUBMIT_MATCHING, logData});
...
/**
 * Create export variable to running function SubmitAdminTeacher
 * @constructor
 */
const SubmitAdminTeacherProcess = new SubmitAdminTeacher()

export default SubmitAdminTeacherProcess