...
class StudentService(BaseService):
    ...
    def set_rule(self, student_dict, operation_type):
        """児童生徒情報のルールを適用する
        ...
        for numeric_rule in numeric_rules:
            student_dict[numeric_rule] = self.to_int_without_exception(
                student_dict[numeric_rule]
            )
        # 配列型変換ルールの適用
        for list_rule in list_rules:
            if student_dict.get(list_rule):
                student_dict[list_rule] = student_dict[list_rule].split(
                    list_splitter
                )
            else:
                student_dict[list_rule] = []
        return student_dict

    def check_master_code_integrity(self, student_dict, master_codes, error_messages):
        """児童生徒一括登録データとマスタとの整合性をチェックする
        ...
        # 学年コードの存在チェック
        grade_code = student_dict.get("grade_code", "")
        if (
